...
def to_tsv(measure, dfs, data_name, alg_names=None, class_type=None):
    ...
    if class_type:
        ...
        fstr = "ss_%s_%s_%s.tsv"%(data_name, measure, class_type)
    else:
        fstr = "ss_%s_%s.tsv"%(data_name, measure)
...
if __name__ == "__main__":
    ...
    for num in range(5, 10):
        alpha = 0.1*num
        alg_abbr = "S({:.1})".format(alpha)
        algs[alg_abbr] = copy.deepcopy(algs["SS"])
        algs[alg_abbr].params.alpha = alpha        
        algs[alg_abbr].abbr = alg_abbr
        names_nonorm.append(alg_abbr)
         
#        alg_abbr = "N({:.1})".format(alpha)
#        algs[alg_abbr] = copy.deepcopy(algs["NSS"])
#        algs[alg_abbr].params.alpha = alpha        
#        algs[alg_abbr].abbr = alg_abbr  
#        names_norm.append(alg_abbr)
    # end of for
    del algs['SS']
    #del algs['NSS']
    
    alg_names = names_nonorm + names_norm
